...
//         <StatCard label="Pending Bills"   value={`₹${(pendingAmt+overdueAmt).toLocaleString('en-IN')}`} icon={FileText} trend={overdueAmt > 0 ? `₹${overdueAmt.toLocaleString('en-IN')} overdue` : 'All good'} trendUp={overdueAmt === 0} delay={180} onClick={() => navigate('/billing')} />
//       </div>
...
//
// The original Dashboard was completely role-blind. It called:
//   reportService.getStatusOverview() → /api/reports/status-overview
//   → Domain.aggregate([{ $match: { tenantId: req.tenantId } }])
//
// For SuperAdmin, req.tenantId is undefined, so $match returns NOTHING.
// Result: SuperAdmin always saw zeros for domains, clients, hosting, revenue.
//
// Fix: The Dashboard now checks user.role at render time.
// - superAdmin → renders <SuperAdminDashboard />  (platform-wide data)
// - admin/staff/client → renders <AdminDashboard /> (tenant-scoped data, same as before)
// ─────────────────────────────────────────────────────────────────────────────
import { useAuth } from '../context/AuthContext'
