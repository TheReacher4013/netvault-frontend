...
import { Globe, Plus, Search, Trash2, Eye, Server } from 'lucide-react'
...
export default function DomainList() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['domains', { search, status, page, perPage }],
    queryFn: () => domainService.getAll({ search, status, page, limit: perPage }),
    ...
    <div className="space-y-5">
      ...
      <Card>
        {domains.length === 0 ? (
          ...
          <>
            {/* Desktop Table — hidden on mobile/tab */}
              ...
              <table className="w-full">
                ...
                <tbody>
                  {domains.map((d, i) => {
                    ...
                    return (
                      <tr key={d._id} className="transition-colors hover:bg-white/[0.02] cursor-pointer"
                        ...
                        <td className="px-4 py-3" onClick={e => e.stopPropagation()}>
                          <div className="flex gap-1.5">
                            <button onClick={() => navigate(`/domains/${d._id}`)}
                              className="p-1.5 rounded-lg transition-colors hover:bg-white/10" title="View"
            ...
            <div className="lg:hidden divide-y" style={{ borderColor: theme.border }}>
              {domains.map((d, i) => {
                ...
                const srNo = (page - 1) * perPage + i + 1
                return (
                  <div key={d._id} className="p-4 hover:bg-white/[0.02] transition-colors cursor-pointer"
                    onClick={() => navigate(`/domains/${d._id}`)}>
                    <div className="flex items-start justify-between gap-2 mb-3">
                      <div className="flex items-center gap-2 min-w-0 flex-1">
                        <span className="text-[10px] font-mono px-1.5 py-0.5 rounded flex-shrink-0"
                          style={{ background: `${theme.accent}15`, color: theme.muted }}>#{srNo}</span>
                        <Globe size={13} style={{ color: theme.accent }} className="flex-shrink-0" />
                    ...
                    <div className="grid grid-cols-2 gap-x-4 gap-y-2 mb-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs mt-0.5" style={{ color: theme.accent }}>{d.hostingId?.label || '—'}</p>
                      </div>
                      <div>
                      ...
                      <div>
                        ...
                        <p className="text-sm font-mono font-bold mt-0.5" style={{
                          color: dl < 0 ? '#C94040' : dl <= 7 ? '#C94040' : dl <= 30 ? '#F0A045' : '#62B849'
                        }}>
                          {dl < 0 ? `${Math.abs(dl)}d ago` : `${dl}d`}
                        </p>
          ...
          {totalPages > 1 && (
            <div className="flex items-center gap-1.5">
              <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1}
                className="px-2.5 py-1 rounded-lg text-xs font-mono transition-colors disabled:opacity-30"
