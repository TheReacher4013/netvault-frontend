...
import { domainService, clientService, whoisService } from '../../services/api'
import { useAuth } from '../../context/AuthContext'
import { Button, Card, CardHeader, StatusBadge, Loader, PageHeader, Input, Select } from '../../components/ui/index'
import DomainMonitoringCard from './DomainMonitoringCard'
import { Edit3, Save, X, Wifi, ArrowLeft, RefreshCw , Layers, ChevronRight, Globe} from 'lucide-react'
...
export default function DomainDetail() {
  ...
  const whoisMut = useMutation({
    ...
    onSuccess: () => {
      toast.success('WHOIS refreshed from registry')
      qc.invalidateQueries(['domain', id])
    },
    onError: (err) => toast.error(err.response?.data?.message || 'WHOIS lookup failed'),
  })
    ...
    <div className="space-y-5 max-w-4xl">
      <PageHeader
        ...
        actions={
          <div className="flex gap-2 flex-wrap">
            ...
            <Button variant="secondary" size="sm" onClick={() => navigate(`/domains/${id}/dns`)}>
              <Wifi size={13} />DNS Manager
            </Button>
            ...
            {!editing
              ...
              : <>
                <Button size="sm" loading={updateMut.isPending} onClick={() => updateMut.mutate(form)}>
                  <Save size={13} />Save
                </Button>
      ...
      <div className="grid lg:grid-cols-3 gap-5">
        <Card className="lg:col-span-2">
            ...
            {editing ? (
              <>
                ...
                <Input label="Expiry Date" name="expiryDate" type="date"
                  value={form.expiryDate?.slice(0, 10) || ''} onChange={handleChange} />
                <Select label="Assign Client" name="clientId"
                  value={form.clientId?._id || form.clientId || ''} onChange={handleChange}>
                  <option value="">No client</option>
                  {clients.map(c => <option key={c._id} value={c._id}>{c.name}</option>)}
                </Select>
                <Input label="Renewal Cost (₹)" name="renewalCost" type="number"
                  value={form.renewalCost || ''} onChange={handleChange} />
              ...
              [
                ...
                <div key={f.key}>
                  ...
                  <p className={`text-sm font-semibold ${f.mono ? 'font-mono' : ''}`} style={{ color: theme.text }}>
                    {domain[f.key] !== undefined && domain[f.key] !== ''
                      ? (f.format ? f.format(domain[f.key]) : domain[f.key])
                      : '—'}
                  </p>
                </div>
              ))
          ...
          {/* WHOIS block */}
          {domain.whois && (domain.whois.registrantName || domain.whois.registrantOrg || domain.whois.lastFetched) && (
            <div className="p-5" style={{ borderTop: `1px solid ${theme.border}` }}>
              <p className="text-[10px] font-mono uppercase tracking-widest mb-3" style={{ color: theme.muted }}>
                WHOIS {domain.whois.lastFetched && `· Updated ${format(new Date(domain.whois.lastFetched), 'dd MMM yyyy')}`}
              </p>
              <div className="grid sm:grid-cols-2 gap-3 text-xs">
                {[
                  ['Registrant Name', domain.whois.registrantName],
                  ['Registrant Org', domain.whois.registrantOrg],
                  ['Registrant Email', domain.whois.registrantEmail],
                  ['Record Updated', domain.whois.updatedDate ? format(new Date(domain.whois.updatedDate), 'dd MMM yyyy') : null],
                ].map(([k, v]) => (
                  <div key={k}>
        ...
        <div className="space-y-4">
            ...
            <div className="mt-4 space-y-2">
              <div className="flex justify-between text-xs">
                ...
                <span className="font-mono font-bold" style={{
                  color: daysLeft <= 7 ? '#C94040' : daysLeft <= 30 ? '#F0A045' : theme.accent
                }}>
          ...
          <DomainMonitoringCard domain={domain} />
          ...
          <table className="w-full">
            ...
            <tbody>
              {(!domain.dnsRecords || domain.dnsRecords.length === 0) && (
                <tr><td colSpan={4} className="text-center py-8 text-xs" style={{ color: theme.muted }}>
                  No DNS records. <button onClick={() => navigate(`/domains/${id}/dns`)} className="underline" style={{ color: theme.accent }}>Add records →</button>
                </td></tr>
              )}
              {domain.dnsRecords?.map(r => (
                <tr key={r._id} className="hover:bg-white/[0.02]" style={{ borderBottom: `1px solid ${theme.border}` }}>
                  <td className="px-4 py-2.5">
                    <span className="text-[10px] font-mono px-2 py-0.5 rounded" style={{ background: `${theme.accent}15`, color: theme.accent }}>{r.type}</span>
                  </td>
      ...
      {(domain.parentDomainId || (domain.subdomains && domain.subdomains.length > 0)) && (
        <Card>
          ...
          <div className="p-5 space-y-3">
            ...
            {/* Show parent if this is a subdomain */}
            ...
            {/* Show subdomains if this is a main domain with children */}
            {domain.subdomains && domain.subdomains.length > 0 && (
              <div>
                <p className="text-[10px] font-mono uppercase mb-2" style={{ color: theme.muted }}>
