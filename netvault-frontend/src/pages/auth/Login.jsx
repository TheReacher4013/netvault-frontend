...
export default function Login() {
  ...
  const afterLogin = (user) => {
    if (rememberMe) localStorage.setItem('nv_remember_email', email.trim().toLowerCase())
    else localStorage.removeItem('nv_remember_email')
  ...
  return (
    <div className="min-h-screen flex items-center justify-center relative overflow-hidden transition-all duration-500"
      ...
      <div className="relative z-10 w-full max-w-5xl mx-auto px-8 flex items-center gap-16 py-12">
                    ...
                    <div className="flex items-center justify-between">
                      <label className="flex items-center gap-2.5 cursor-pointer select-none">
                        <div onClick={() => setRememberMe(v => !v)}
                          ...
                          style={{ background: rememberMe ? theme.accent : 'transparent', border: `1.5px solid ${rememberMe ? theme.accent : theme.border}`, cursor: 'pointer' }}>
                          {rememberMe && (
