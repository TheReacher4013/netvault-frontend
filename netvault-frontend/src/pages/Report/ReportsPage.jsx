...
function EmailScheduleSection() {
  ...
  const handleSave = async () => {
    ...
    try { await reportDataAPI.saveEmailSchedule({ emails, sendTime, enabled }); setMsg('✅ Schedule saved! Reports will be sent daily at ' + sendTime + ' IST'); }
...
export default function ReportsPage() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try { const res = await reportDataAPI.getAdminSummary(); setData(res.data?.data || null); }
  ...
  const growthStr = growth != null ? (growth >= 0 ? `↑ ${growth}% vs last month` : `↓ ${Math.abs(growth)}% vs last month`) : null;
      ...
      {loading ? (
        ...
        <>
          <div className="rp-stats">
            <StatCard icon="👥" label="Total Clients" value={d.clients?.total} color="#6366F1" sub={`${d.clients?.active || 0} active`} />
            <StatCard icon="👤" label="Staff Members" value={d.staff?.total} color="#8B5CF6" />
            <StatCard icon="🌐" label="Domains" value={d.domains?.total} color="#3B82F6" sub={d.domains?.expiring ? `${d.domains.expiring} expiring` : null} subColor="#D97706" />
            <StatCard icon="🖥️" label="Hosting Plans" value={d.hosting?.total} color="#06B6D4" sub={d.hosting?.expiring ? `${d.hosting.expiring} expiring` : null} subColor="#D97706" />
            <StatCard icon="🧾" label="Total Invoices" value={inv.total} color="#F59E0B" />
            <StatCard icon="💰" label="Total Revenue" value={fmtMoney(inv.totalRevenue)} color="#16A34A" sub={growthStr} subColor={growthColor} />
            <StatCard icon="📅" label="This Month" value={fmtMoney(inv.thisMonthRevenue)} color="#6366F1" />
            <StatCard icon="⚠️" label="Overdue" value={fmtMoney(inv.overdueRevenue)} color="#DC2626" sub={`${inv.overdue || 0} invoices`} subColor="#DC2626" />
          </div>
