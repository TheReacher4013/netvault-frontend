...
export default function HostingList() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['hosting', search, page, perPage],
    queryFn: () => hostingService.getAll({ search, page, limit: perPage }),
      ...
      <Card className="p-4 flex gap-3">
        <div className="flex items-center gap-2 flex-1 px-3 py-2 rounded-xl"
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search hosting..."
            className="bg-transparent outline-none text-xs flex-1" style={{ color: theme.text }} />
        </div>
      </Card>
                        ...
                        <td className="px-4 py-3">
                          <div>
                            <p className="text-xs font-mono" style={{ color: theme.muted }}>
                              {format(new Date(h.expiryDate), 'dd MMM yyyy')}
                            </p>
